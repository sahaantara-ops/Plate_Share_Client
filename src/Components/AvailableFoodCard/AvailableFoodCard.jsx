import React from 'react';
import { Link } from 'react-router';

const AvailableFoodCard = ({models}) => {
    
         const { title,image, category, description, expiryDate,_id, quantity,pickup_location,food_status} = models
    console.log(models);
  return (
    <div className="card bg-base-100 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 mr-10">
      <figure className="h-48 overflow-hidden">
        <img
          src={image}
          alt={image}
          className="w-full h-full object-cover hover:scale-110 transition-transform duration-300"
        />
      </figure>
      <div className="card-body">
        <h2 className="card-title">{title}</h2>
        <div className="badge text-xs badge-xs badge-secondary rounded-full">{category}</div>
        <div className="text-xs text-secondary">{quantity}</div>
        <p className="line-clamp-1">
            {description}
        </p>
        <div className="text-xs text-secondary">{expiryDate}</div>
        <div className="text-xs text-secondary">{food_status}</div>
        <div className="text-xs text-secondary">{pickup_location}</div>
        <div className="card-actions justify-between items-center mt-4">
          <div className="flex gap-4 text-sm text-base-content/60">
           
            
          </div>
          <Link to={`/model-details/${_id}`} className="btn rounded-full bg-linear-to-r from-pink-500 to-red-600 hover:from-red-600 hover:to-pink-500 text-white w-full btn-sm">View Details</Link>
        </div>
      </div>
    </div>
    );
};

export default AvailableFoodCard;